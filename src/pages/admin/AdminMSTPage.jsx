...
const AdminMSTPage = () => {
  ...
  const [testName, setTestName] = useState('MST-1');
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/mst/upload', formData);
      setResult(data);
      toast.success(data.message || 'MST results uploaded successfully');
      setFile(null);
      fetchData();
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to upload MST results');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (testName, semester, testDate) => {
    ...
    try {
      await api.delete('/mst/upload', { data: { testName, semester, testDate } });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <header className="flex flex-col md:flex-row md:items-center gap-6 glass-card p-8 rounded-3xl mb-8">
        ...
        <div>
          <h1 className="text-3xl md:text-4xl font-display font-black tracking-tight text-[var(--text-primary)]">MST Results Dashboard</h1>
          <p className="text-[var(--text-secondary)] font-medium mt-2">Upload MST Excel sheets by semester and view students' scores dynamically.</p>
        </div>
      </header>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-1 space-y-8">
          <div className="glass-card p-8 space-y-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-widest text-[var(--text-secondary)] mb-2">MST Type</label>
                <select
                  value={testName}
                  onChange={(e) => setTestName(e.target.value)}
                  className="w-full bg-[var(--bg-input)] text-[var(--text-primary)] border border-[var(--border-light)] rounded-xl p-3 text-sm focus:outline-none focus:border-indigo-500"
                >
                  <option className="bg-[var(--bg-app)] text-[var(--text-primary)]" value="MST-1">MST-1</option>
                  <option className="bg-[var(--bg-app)] text-[var(--text-primary)]" value="MST-2">MST-2</option>
                </select>
              </div>
            </div>
          ...
          <div className="bg-indigo-50 border border-indigo-200 shadow-sm rounded-2xl p-6 flex gap-4">
            ...
            <div className="text-xs text-indigo-900 leading-relaxed font-medium">
              ...
              <ul className="list-disc pl-4 space-y-1">
                ...
                <li>All other columns (e.g. subject names) are treated as subject scores.</li>
