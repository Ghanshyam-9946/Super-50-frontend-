...
const STATUS_META = {
  open: { label: 'Open', icon: Inbox, color: 'text-slate-500', ring: 'bg-slate-500/10 border-slate-500/20', dot: '#64748b' },
  started: { label: 'Started', icon: PlayCircle, color: 'text-indigo-500', ring: 'bg-indigo-500/10 border-indigo-500/20', dot: '#6366f1' },
  pending: { label: 'Pending', icon: PauseCircle, color: 'text-amber-500', ring: 'bg-amber-500/10 border-amber-500/20', dot: '#f59e0b' },
  rejected: { label: 'Rejected', icon: XCircle, color: 'text-red-500', ring: 'bg-red-500/10 border-red-500/20', dot: '#ef4444' },
  completed: { label: 'Completed', icon: CheckCircle2, color: 'text-emerald-500', ring: 'bg-emerald-500/10 border-emerald-500/20', dot: '#10b981' },
  forwarded: { label: 'Forwarded', icon: Share2, color: 'text-purple-500', ring: 'bg-purple-500/10 border-purple-500/20', dot: '#8b5cf6' },
};

const PRIORITY_META = {
  high: { label: 'High', accent: '#ef4444', ring: 'bg-red-500/10 border-red-500/20 text-red-500' },
  medium: { label: 'Medium', accent: '#f59e0b', ring: 'bg-amber-500/10 border-amber-500/20 text-amber-500' },
  low: { label: 'Low', accent: '#10b981', ring: 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' },
};
...
const ACTION_META = {
  started: { label: 'Start', icon: PlayCircle, cls: 'text-indigo-500 hover:bg-indigo-500/10 border-indigo-500/20', prompt: 'Add a comment about starting this task…' },
  pending: { label: 'Pending', icon: PauseCircle, cls: 'text-amber-500 hover:bg-amber-500/10 border-amber-500/20', prompt: 'Why is this task on hold?' },
  completed: { label: 'Complete', icon: CheckCircle2, cls: 'text-emerald-500 hover:bg-emerald-500/10 border-emerald-500/20', prompt: 'Add a completion note…' },
  rejected: { label: 'Reject', icon: XCircle, cls: 'text-red-500 hover:bg-red-500/10 border-red-500/20', prompt: 'Reason for rejecting this task…' },
};
...
export default function FacultyTasksPage() {
  ...
  const loadFaculty = useCallback(async () => {
    try {
      ...
      if (data.success) setFaculty(data.data.filter((f) => f._id !== user?._id));
    } catch {
      /* silent */
    }
  }, [user?._id]);
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      await Promise.all([loadStats(), loadFaculty(), loadAssigned(), loadAllocated()]);
      setLoading(false);
    })();
  ...
  const statCards = useMemo(
    () => [
      { key: 'open', ...STATUS_META.open, value: stats?.open || 0, sub: 'Open Task' },
      { key: 'started', ...STATUS_META.started, value: stats?.started || 0, sub: 'Started Task' },
      { key: 'pending', ...STATUS_META.pending, value: stats?.pending || 0, sub: 'Pending' },
      { key: 'rejected', ...STATUS_META.rejected, value: stats?.rejected || 0, sub: 'Reject' },
      { key: 'completed', ...STATUS_META.completed, value: stats?.completed || 0, sub: 'Completed' },
      { key: 'forwarded', ...STATUS_META.forwarded, value: stats?.forwarded || 0, sub: 'Forwarded' },
    ],
    [stats]
  );
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {statCards.map((c, i) => (
          ...
          >
            <div className={`w-10 h-10 rounded-xl border flex items-center justify-center mb-4 ${c.ring}`}>
              <c.icon size={18} className={c.color} />
            </div>
            <div className="text-3xl font-display font-black text-[var(--text-primary)] leading-none">{c.value}</div>
            <div className="text-[11px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mt-2">{c.sub}</div>
          </motion.div>
        ))}
      </div>
        ...
        {tabs.map((t) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <motion.div layoutId="taskTabUnderline" className="absolute -bottom-[5px] left-3 right-3 h-[3px] rounded-full bg-[var(--primary)]" />
              )}
            </button>
          );
        })}
      ...
      {loading ? (
        <div className="glass-card p-16 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[var(--text-secondary)] font-medium text-sm">Loading task board…</p>
        </div>
      ) : (
        <AnimatePresence mode="wait">
          ...
          >
            {tab === 'assigned' && <AssignedTasks tasks={assigned} faculty={faculty} onChange={refreshAll} />}
            {tab === 'add' && (
              <AddTaskForm
                faculty={faculty}
                onCreated={async () => {
  ...
  if (tasks.length === 0) {
    return (
      <div className="glass-card p-16 text-center flex flex-col items-center gap-3 rounded-3xl">
        ...
        <p className="text-[var(--text-primary)] font-bold">No tasks assigned to you yet</p>
        <p className="text-[var(--text-secondary)] text-sm">Allocated tasks will appear here, grouped by priority.</p>
      </div>
    );
  }
      ...
      {['high', 'medium', 'low'].map((p) => (
        <section key={p} className="space-y-4">
          <div className="flex items-center gap-3">
            <span className={`badge ${PRIORITY_META[p].ring}`} style={{ borderColor: PRIORITY_META[p].accent + '33' }}>
              <Flag size={12} /> {PRIORITY_META[p].label} Priority
            </span>
            <span className="text-xs font-bold text-[var(--text-secondary)]">{groups[p].length} task{groups[p].length !== 1 ? 's' : ''}</span>
            <div className="flex-1 h-px bg-[var(--border-light)]" />
          </div>

          {groups[p].length === 0 ? (
            <p className="text-sm text-[var(--text-secondary)] pl-1 italic">No {PRIORITY_META[p].label.toLowerCase()} priority tasks.</p>
          ) : (
            <div className="grid gap-4">
              {groups[p].map((task) => (
                <TaskCard key={task._id} task={task} faculty={faculty} onChange={onChange} />
              ))}
            </div>
          )}
        </section>
...
function TaskCard({ task, faculty, onChange }) {
  ...
  const confirmStatus = async (newStatus) => {
    if (!comment.trim()) return toast.error('Please add a comment before continuing');
    setBusy(true);
    try {
  ...
  return (
    <div className="glass-card p-5 rounded-2xl" style={{ borderLeft: `3px solid ${PRIORITY_META[task.priority]?.accent || '#8b5cf6'}` }}>
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-base font-display font-bold text-[var(--text-primary)]">{task.title}</h3>
          ...
          <div className="flex items-center gap-4 flex-wrap mt-3 text-xs text-[var(--text-secondary)] font-medium">
            {task.createdBy && (
              <span className="flex items-center gap-1.5"><UserCircle2 size={14} /> By {task.createdBy.name}</span>
            )}
            {deadline && (
              <span className="flex items-center gap-1.5"><Calendar size={14} /> Due {deadline}</span>
            )}
            {task.forwardedTo && (
              <span className="flex items-center gap-1.5 text-purple-500"><Share2 size={14} /> → {task.forwardedTo.name}</span>
            )}
            {task.referenceFile && (
              <a
          ...
          {task.statusHistory?.length > 0 && (
            <div className="mt-3 space-y-1.5">
              {task.statusHistory.slice(-2).reverse().map((h, i) => (
                <div key={i} className="flex items-start gap-2 text-xs text-[var(--text-secondary)] bg-[var(--bg-hover)] rounded-lg px-2.5 py-1.5">
                  <span className={`font-bold shrink-0 ${STATUS_META[h.status]?.color || ''}`}>{STATUS_META[h.status]?.label || h.status}:</span>
        ...
        {mode && mode !== 'forward' && (
          ...
          >
            <div className="mt-4 pt-4 border-t border-[var(--border-light)] flex flex-col sm:flex-row gap-3">
              ...
              >
                {busy ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle2 size={14} />} Confirm {ACTION_META[mode]?.label}
              </button>
            </div>
          </motion.div>
        )}
          ...
          >
            <div className="mt-4 pt-4 border-t border-[var(--border-light)] flex flex-col gap-3">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                >
                  {busy ? <Loader2 size={14} className="animate-spin" /> : <Send size={14} />} Forward
                </button>
              </div>
            </div>
          </motion.div>
...
function AddTaskForm({ faculty, onCreated }) {
  ...
  const labelCls = 'block text-[11px] font-black text-[var(--text-secondary)] uppercase tracking-widest mb-2';
    ...
    <div className="glass-card p-8 rounded-3xl max-w-4xl">
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          <label className={labelCls}>Faculty <span className="text-[var(--primary)]">(select one or more)</span></label>
          <div className="border border-[var(--border-light)] rounded-2xl overflow-hidden">
            ...
            <div className="max-h-52 overflow-y-auto custom-scrollbar p-2 space-y-1">
              {filtered.length === 0 && (
                <p className="text-sm text-[var(--text-secondary)] text-center py-6">No faculty found.</p>
              )}
              {filtered.map((f) => {
                ...
                return (
                  ...
                  >
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center shrink-0 ${on ? 'bg-[var(--primary)] border-[var(--primary)]' : 'border-[var(--border-light)]'}`}>
                      {on && <CheckCircle2 size={14} className="text-white" />}
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-[var(--text-primary)] truncate">{f.name}</div>
                      <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-secondary)]">
                        {f.role}{f.department ? ` • ${f.department}` : ''}
                      </div>
                    </div>
                  </button>
                );
        ...
        <div className="md:col-span-2">
          ...
          <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Prepare Semester 5 result analysis" className={inputCls} />
        </div>

        <div className="md:col-span-2">
          <label className={labelCls}>Task Description</label>
          <textarea rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe what needs to be done…" className={inputCls + ' resize-none'} />
        </div>
            ...
            {['high', 'medium', 'low'].map((p) => (
              <button
                ...
                className={`flex-1 px-3 py-3 rounded-xl border text-xs font-bold transition-all ${
                  form.priority === p ? PRIORITY_META[p].ring : 'border-[var(--border-light)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'
                }`}
                style={form.priority === p ? { borderColor: PRIORITY_META[p].accent } : {}}
              >
        ...
        <div className="md:col-span-2">
          <label className={labelCls}>Reference File / Format <span className="opacity-60 normal-case tracking-normal font-medium">(PDF, DOC, PPT, XLS, ZIP — optional)</span></label>
          <label className="flex items-center gap-3 border border-dashed border-[var(--border-light)] rounded-xl px-4 py-3 cursor-pointer hover:border-[var(--primary)] transition-colors">
            ...
            <span className="text-sm text-[var(--text-secondary)] truncate">{file ? file.name : 'Attach a reference file or format'}</span>
            <input
              type="file"
              className="hidden"
            ...
            />
            {file && (
              <button onClick={(e) => { e.preventDefault(); setFile(null); }} className="ml-auto text-[var(--text-secondary)] hover:text-red-500">
                <X size={16} />
              </button>
            )}
          </label>
      ...
      <div className="flex justify-end gap-3 mt-8 pt-6 border-t border-[var(--border-light)]">
        <button onClick={submit} disabled={submitting} className="btn-premium flex items-center gap-2 text-sm">
          {submitting ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Allocate Task
        </button>
      </div>
    </div>
  );
    ...
    <div className="space-y-5">
      <div className="glass-card p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-2 text-sm font-bold text-[var(--text-primary)]">
          <Filter size={16} className="text-[var(--primary)]" /> View any faculty's tasks
        </div>
        <select
          value={filterId}
      ...
      {tasks.length === 0 ? (
        <div className="glass-card p-16 text-center flex flex-col items-center gap-3 rounded-3xl">
          ...
          <p className="text-[var(--text-primary)] font-bold">No allocated tasks found</p>
        </div>
      ) : (
