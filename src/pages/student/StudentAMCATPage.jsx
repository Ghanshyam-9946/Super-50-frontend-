...
const StudentAMCATPage = () => {
  ...
  const fetchResults = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/amcat/my-result');
      setResults(data.data || []);
    } catch (error) {
      toast.error('Failed to load AMCAT results');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-sm font-medium text-[var(--text-secondary)]">Loading your AMCAT report...</p>
      </div>
    );
  }
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-2xl font-display font-black text-[var(--text-primary)]">No AMCAT Results Yet</h2>
          <p className="text-[var(--text-secondary)] text-sm max-w-md mx-auto">
            Your AMCAT assessment results haven't been uploaded by the admin for your semester yet. Once uploaded, your subject-wise scores will appear here.
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 glass-card p-8 rounded-3xl mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-display font-black tracking-tight text-[var(--text-primary)]">AMCAT Performance Report</h1>
...
export default StudentAMCATPage;
