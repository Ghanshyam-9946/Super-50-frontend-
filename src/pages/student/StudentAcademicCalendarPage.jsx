...
// (Excel with all its sheets and colours, or PDF) — see
// AcademicCalendarViewer.jsx. Shared by three routes — the student route
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-6">
      <header className="glass-card flex flex-col md:flex-row md:items-center justify-between gap-4 p-8 rounded-3xl">
        ...
        {file && (
          ...
          >
            <Download size={16} /> Download {file.type === 'pdf' ? 'PDF' : 'Excel'}
          </button>
        )}
      </header>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-3">
            {file && (
              <div className="flex items-center gap-2 text-xs font-bold text-[var(--text-secondary)] px-1">
                {file.type === 'pdf' ? <FileText size={14} /> : <FileSpreadsheet size={14} />} {file.name}
              </div>
            )}
            <div className="glass-card p-2 md:p-3 rounded-3xl">
              <AcademicCalendarViewer calendar={calendar} height="80vh" />
            </div>
          </motion.div>
        </>
      )}
